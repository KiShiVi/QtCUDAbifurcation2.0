...
__host__ void LyapunovSpectrum_1D(
	double				in_tMax,
	double				in_NT,
	int					in_nPts,
	double				in_h,
	...
	std::atomic<int>& progress)
{
	...
	for (size_t i = 0; i < amountOfIteration; ++i)
	{
		...
		h_data = (double*)malloc(nPtsLimiter * amountOfTPoints * sizeof(double));
		h_dataSizes = (int*)malloc(nPtsLimiter * sizeof(int));
		...
		LyapunovSpectrumKernel << <gridSize, blockSize >> > (
			nPtsLimiter,
			in_tMax,
			in_NT,
			in_h,
		...
		for (size_t i = 0; i < nPtsLimiter; ++i) {
			...
			outFileStream << h_dataTimes[i] << ", ";

			for (size_t j = 0; j < h_dataSizes[i]; ++j) {
				
				if (outFileStream.is_open())
				{
					outFileStream << h_data[i * amountOfTPoints + j] << ", ";
				}
				else
				{
					std::cout << "\nOutput file open error" << std::endl;
...
)
{
	...
	{
		z[i] = 0.5 * (sinf(idx * (i * idx + 1)));	// 0.2171828 change to z[i] = rand(0, 1) - 0.5;
		zPower += z[i] * z[i];
	}
	...
	for (int i = 0; i < amountOfTPoints; ++i)
	{
		...
		double in_data_1 = calculationForLLE(x, y, eps, AmountDim);
		in_data[index + i] = (float)in_data_1;
		in_data_1 = (1 / in_data_1);
...
__global__ void LyapunovSpectrumKernel(
	int in_nPts,
	double in_TMax,
	double in_NT,
	double in_h,
...
)
{
	...
	int AmountDim = 3;
	////// Change to dynamic / KISH
	double* x = new double[AmountDim] { in_initialConditions[0], in_initialConditions[1], in_initialConditions[2] }; 
	double* y = new double[AmountDim]; 
	//double* X_1 = new double[AmountDim];  //in_data[index + AmountDim + i]
	//double* Z_1 = new double[AmountDim]; //in_data[index + 2*AmountDim + i]
	//double* M = new double[AmountDim * AmountDim]; //in_data[index + 5*AmountDim + i]
	//double* OUT = new double[AmountDim]; //in_data[index + 3*AmountDim + i]
	//double* VNorm = new double[AmountDim]; //in_data[index + 4*AmountDim + i]
	...
	for (int i = 0; i < AmountDim * AmountDim; i++)
	{
		in_data[index + 5 * AmountDim + i] = 0.5 * (sinf(idx * (i * idx + 1)));	// 0.2171828 change to z[i] = rand(0, 1) - 0.5;
	}
	...
	for (size_t i = 0; i < amountOfSkipPoints; ++i) {
		...
		{
			in_dataSizes[idx] = 0;
			delete[] localParam; //delete[] VNorm; delete[] OUT;
			delete[] x; delete[] y; //delete[] X_1; delete[] Z_1; delete[] M;
			return;
		}
	}

	//Calculating
	GSR(index, in_data, AmountDim);
	//GSR(M, X_1, Z_1, AmountDim, VNorm);

	for (int i = 0; i < AmountDim; ++i) {
		for (int j = 0; j < AmountDim; ++j) {
				in_data[index + 5 * AmountDim + i * AmountDim + j] = in_data[index + 5 * AmountDim + i * AmountDim + j] *eps + x[j];
		}
	...
	for (int i = 0; i < amountOfTPoints; ++i)
	{
		...
		for (int j = 0; j < amountOfNT_points; ++j) {
			...
			for (int w = 0; w < AmountDim; ++w) {
				
				for (int g = 0; g < AmountDim; ++g)
					y[g] = in_data[index + 5 * AmountDim + w * AmountDim + g];
				...
				for (int g = 0; g < AmountDim; ++g)
					in_data[index + 5 * AmountDim + w * AmountDim + g] = y[g];
					//M[w * AmountDim + g] = y[g];
			}
		...
		for (int w = 0; w < AmountDim; ++w) {
			for (int j = 0; j < AmountDim; ++j) {
				in_data[index + 5 * AmountDim + w * AmountDim + j] = in_data[index + 5 * AmountDim + w * AmountDim + j] - x[j];
				//M[w * AmountDim + j] = M[w * AmountDim + j] - x[j];
			}
		}
		...
		for (int n = 0; n < AmountDim; ++n)
			in_data[index + n] = in_data[index + n] + log(in_data[index + 4 * AmountDim + n]/eps)/in_TMax; //(float)(log(VNorm[n] / eps));


		for (int w = 0; w < AmountDim; ++w) {
			for (int j = 0; j < AmountDim; ++j) {
				in_data[index + 5 * AmountDim + w * AmountDim + j] = in_data[index + 5 * AmountDim + w * AmountDim + j] * eps + x[j];
				//M[w * AmountDim + j] = M[w * AmountDim + j] * eps + x[j];
			}
		}
	...
	//for (int n = 0; n < AmountDim; ++n)
	//	in_data[index + n] = OUT[n]; //(float)(log(VNorm[n] / eps));

	delete[] localParam; //delete[] VNorm; delete[] OUT;
	delete[] x; delete[] y; //delete[] X_1; delete[] Z_1; delete[] M;

	//in_dataSizes[idx] = AmountDim;
...
__device__ double calculationForLLE(double* X, double* Y, double eps, int N)
{
	double res = 0;
	for (int i = 0; i < N; i++)
...
//__device__ double ArraysMult(double* X, double* Y, int N) {
//	double result = 0;
//	for (int i = 0; i < N; i++)
//		result = result + X[i] * Y[i];		//		result = result + X[i] * Y[i];
//	return result;
//}

__device__ void ArrayMultScalar(size_t index, double* in_data, double Y, int N) {
	for (int i = 0; i < N; i++)
		in_data[index + N + i] = in_data[index + N + i] * Y; // X[i] = X[i] * Y;
}

//__device__ double VectorNorm(size_t index, double* in_data, int N)
//{
//	double res = 0;
//
//	for (int i = 0; i < N; i++)
//		res = res + in_data[index + N + i] * in_data[index + N + i]; //		res = res + X[i] * X[i];
//
//	res = sqrtf(res);
//
//	return res;
...
//double* X_1 = new double[AmountDim];  //in_data[index + N + i]
//double* Z_1 = new double[AmountDim]; //in_data[index + 2*N + i]
//double* M = new double[AmountDim * AmountDim]; //in_data[index + 5*N + i]
//double* OUT = new double[AmountDim]; //in_data[index + 3*N + i]
//double* VNorm = new double[AmountDim]; //in_data[index + 4*N + i]

//__device__ void GSR(double* M, double* X, double* Z, int N, double* VNorm) {

__device__ void GSR(size_t index, double* in_data, int N) {
	double result = 0;
	...
	for (int i = 0; i < N; i++) {
		...
		for (int k = 0; k < N; k++)
			in_data[index + 2 * N + k] = in_data[index + 5 * N + i * N + k];

		for (int k = 0; k < i; k++) {
			for (int j = 0; j < N; j++)
				in_data[index + N + j] = in_data[index + 5 * N + j + k * N];
			result = 0;
			for (int i = 0; i < N; i++)
				result = result + in_data[index + N + i] * in_data[index + 2 * N + i];

			ArrayMultScalar(index, in_data, result, N);

			for (int j = 0; j < N; j++)
				in_data[index + 2 * N + j] = in_data[index + 2 * N + j] - in_data[index + N + j];
		}

		result = 0;

		for (int i = 0; i < N; i++)
			result = result + in_data[index + 2 * N + i] * in_data[index + 2 * N + i]; //		VNorm[i] = VectorNorm(Z, N);
		result = sqrtf(result);


		//VNorm[i] = VectorNorm(Z, N);
		in_data[index + 4 * N + i] = result;
		double VecNorm = 1 / result;

		for (int k = 0; k < N; k++)
			in_data[index + 5 * N + i * N + k] = in_data[index + 2 * N + k] * VecNorm;
	}
...
//}
//
//__device__ void ArrayMultScalar(double* X, double Y, int N) {
//	for (int i = 0; i < N; i++)
//		X[i] = X[i] * Y;
//}
//
//__device__ double VectorNorm(double* X, int N)
//{
//	double res = 0;
//
//	for (int i = 0; i < N; i++)
...
//				X[j] = M[j + k * N];
//
//			ArrayMultScalar(X, ArraysMult(Z, X, N), N);
//
//			for (int j = 0; j < N; j++)
//				Z[j] = Z[j] - X[j];
//		}
